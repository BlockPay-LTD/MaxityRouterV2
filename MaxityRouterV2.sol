...
contract MaxityRouterV2 is Ownable,IMRouter,ReentrancyGuard{
    ...
    function mintAndSellMax721(address _token,address designer
        ...
        IMaxityMarketPlace(marketnew).sellByFixPrice(_token,new_tokenids,unit_prices,ngoAddress);
        return new_tokenids;
    }
    ...
    function mintAndAuctionMax721(address _token,address designer
        ...
        uint256 [] memory new_tokenids =  IMaxity721Token(_token).mint(designer, marketnew, _futureRoyaltys, tokenURIOrigins);
        address ngoAddress=IMaxityMetadata(_token).ngowallet();
        IMaxityMarketPlace(marketnew).auctionMax721(_token,new_tokenids,base_prices,incre_prices,starttimes,deadlines,ngoAddress);
        return new_tokenids;
    }
